...
class TC8CocoDataset(CustomDataset):
    """
    Dataset for TownCentre evaluating the orientation accuracy with 8 orientation classes.
    Specifically made for the HPN (ICIP) model.
    The dataset classes MUST be only 1 (person), since the Fast R-CNN classifier, used to classify background vs. person,
    only accepts 1 class, and mmdet would not compile if this dataset had more than 1 class.


    Takes care of mapping the orientation classes from the orientation of the subjects,
    as a workaround for the aforementioned problem
    """
    ...
    def load_annotations(self, ann_file):
        ...
        # posedPath = ann_file
        path_split = os.path.split(ann_file)
        posedAnn = os.path.join(path_split[0], 'posed_8_'+path_split[1])
        self.cocoPosed = COCO(posedAnn)
        self.pose_dict = [
            dict(id=0, name='[-180,-135]'),
            dict(id=1, name='(-135,-90]'),
            dict(id=2, name='(-90,-45]'),
            dict(id=3, name='(-45,0]'),
            dict(id=4, name='(0,45]'),
            dict(id=5, name='(45,90]'),
            dict(id=6, name='(90,135]'),
            dict(id=7, name='(135,180]')
        ]
        self.pose_ids = [0,1,2,3,4,5,6,7]
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            ## orientation
            gt_orientations.append(ann['head_orientation'])
        ...
        if gt_orientations:
            gt_orientations = np.array(gt_orientations, dtype=np.int64)
    ...
    def _pose2json(self, results):
        """Convert pose results to COCO json style.
        results: It's a list of lists, one list per image
        ...
        for idx in range(len(self)):
            ...
            for orientation in range(len(result)):
                bboxes = result[orientation]
                for i in range(bboxes.shape[0]):
                    data = dict()
                    data['image_id'] = img_id
                    # data['bbox'] = bboxes[i]
                    data['bbox'] = self.xyxy2xywh(bboxes[i])
                    data['score'] = float(bboxes[i][4])
                    # data['orientation'] = self.cat_ids[orientation]
                    data['orientation'] = orientation
                    # needed for coco eval
                    data['category_id'] = orientation
                    json_results.append(data)
        return json_results

    def _pose2bbox2json(self, results):
        """Convert pose results to COCO bbox json style.
        results: It's a list of lists, one list per image
                    Each image is a list with 4 element (1 for each pose direction)
        """
        json_results = []
        for idx in range(len(self)):
            img_id = self.img_ids[idx]
            # new version returns list of orientations, not list of list of orientations
            result = results[idx]
            # result = results[idx][0]
            for orientation in range(len(result)):
                bboxes = result[orientation]
    ...
    def results2json(self, results, outfile_prefix):
        ...
        There are 4 types of results: proposals, bbox predictions, mask, pose
        ...
        if isinstance(results[0], list):
            # could be detection or pose, in our case, try hardcode the pose?
            # json_results = self._det2json(results)
            json_results = self._pose2json(results)
            result_files['bbox'] = f'{outfile_prefix}.bbox.json'
            result_files['pose'] = f'{outfile_prefix}.pose.json'
            result_files['proposal'] = f'{outfile_prefix}.bbox.json'
            # mmcv.dump(json_results, result_files['bbox'])
            mmcv.dump(json_results, result_files['pose'])

            # now do a temp file for mAP without orientation
            json_bbox_results = self._pose2bbox2json(results)
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        Args:
            results (list[tuple | numpy.ndarray]): Testing results of the
                dataset.
                It's a list of lists, one list per image
                    Each image is a list with 1 element, a list of 4 (1 for each pose direction)
    ...
    def evaluate(self,
        ...
        Args:
            results (list[list | tuple]): Testing results of the dataset.
                It's a list with an item for each image
                    Each image has a single element containing the 4 classes:
                        Each element has dimension n*5, n= number of pred for that class
            metric (str | list[str]): Metrics to be evaluated. Options are
                'bbox', 'segm', 'proposal', 'proposal_fast', 'pose'.
        ...
        allowed_metrics = ['bbox', 'segm', 'proposal', 'proposal_fast', 'pose', 'pose_acc']
        ...
        # cocoGt = self.coco
        # we need to create a duplicate dataset, to change annotation format
        # in particular, categories needs to become 4
        # cocoGt.dataset['categories'] = [
        #     dict(id=0, name='[-180,-90]'),
        #     dict(id=1, name='(-90,0]'),
        #     dict(id=2, name='(0,90]'),
        #     dict(id=3, name='(90,180]')
        # ]
        for metric in metrics:
            ...
            cocoGt = self.coco
            if metric == 'pose':
                cocoGt.dataset['categories'] = self.pose_dict.copy()
            else:
                cocoGt.dataset['categories'] = [dict(id=0, name='person')]
            ...
            if metric=='pose':
                cocoEval = COCOeval(self.cocoPosed, cocoDt, iou_type)
                # cocoEval.params.catIds = [0, 1, 2, 3]
                cocoEval.params.catIds = self.pose_ids
            else:
            ...
            if metric == 'proposal':
                ...
                if classwise:  # Compute per-category AP
                    ...
                    if metric == 'pose':
                        ...
                        ids = self.cat_ids
                    assert len(ids) == precisions.shape[2]

                    results_per_category = []
                    # for idx, catId in enumerate(self.cat_ids):
                    # for idx, catId in enumerate(self.pose_ids):
                    for idx, catId in enumerate(ids):
                        # area range index 0: all area ranges
                        # max dets index -1: typically 100 per image
                        if metric == 'pose':
                            nm = self.cocoPosed.loadCats(catId)[0]
                        else:
    ...
    def get_accuracy(self, results):
        ...
        for k in classwise_accuracy:
            ...
            if classwise_accuracy[k]['fp'] + classwise_accuracy[k]['tp'] == 0:
                ...
                classwise_accuracy[k]['precision'] = float(classwise_accuracy[k]['tp']) / (
                    float(classwise_accuracy[k]['fp'] + classwise_accuracy[k]['tp']))
            ...
            if classwise_accuracy[k]['denominator'] == 0:
                ...
                classwise_accuracy[k]['accuracy'] = float(classwise_accuracy[k]['numerator']) / float(
                    classwise_accuracy[k]['denominator'])

            if classwise_accuracy[k]['noMiss_denominator'] == 0:
                classwise_accuracy[k]['noMiss_accuracy'] = 1
            else:
                classwise_accuracy[k]['noMiss_accuracy'] = float(classwise_accuracy[k]['numerator']) / float(
                    classwise_accuracy[k]['noMiss_denominator'])
