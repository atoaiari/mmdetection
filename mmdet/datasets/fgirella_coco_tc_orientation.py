...
class TCCocoDataset(CustomDataset):
    """
        Dataset for TownCentre evaluating the orientation accuracy with 4 orientation classes.
    ...
    def load_annotations(self, ann_file):
        ...
        posedAnn = os.path.join(path_split[0], 'posed_'+path_split[1])
    ...
    def _pose2json(self, results):
        ...
        results: It's a list of lists, one list per image
                    ...
                    In new version is a list with 4 elements
        """
        json_results = []
        for idx in range(len(self)):
            img_id = self.img_ids[idx]
    ...
    def _pose2bbox2json(self, results):
        ...
        results: It's a list of lists, one list per image
                    OLD: Each image is a list with 1 element, a list of 4 (1 for each pose direction)
                    NEW VERSION: Each image is a list with 4 element (1 for each pose direction)
        """
        json_results = []
        for idx in range(len(self)):
            img_id = self.img_ids[idx]
    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            if metric == 'pose_acc':
                ...
                # plt.savefig('CM_ICIP_TC4_480p_InverseLog.png')
