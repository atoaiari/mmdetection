...
dataset_type = 'TC8CocoDataset'
# background class is automatically assigned to be the last one
...
train_pipeline = [
    ...
    dict(type='LoadOrientation', num_orientations=8),
    # dict(type='Resize', img_scale=(1333, 800), keep_ratio=True),
    dict(type='Resize', img_scale=(853, 480), keep_ratio=True),
    dict(type='RandomFlip', flip_ratio=0.0000001),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='RandomFlip', flip_ratio=0.000001),
            dict(type='Normalize', **img_norm_cfg),
            dict(type='Pad', size_divisor=32),
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
             init_kwargs=dict(
                 entity='',
                 project='',
                 name='ICIP_TC8_sequential_480p_LossWeight_classWeight2'
             ),
             interval=100
             )
    ])
...
model = dict(
    ...
    roi_head=dict(
        ...
        orientation_head=dict(
                ...
                reg_class_agnostic=False,
                loss_cls=dict(
                    type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[3.86, 3.82, 2.45, 1.22, 3.30, 3.47, 2.56, 1.49]),
                loss_bbox=dict(type='L1Loss', loss_weight=1.0))
    ),
